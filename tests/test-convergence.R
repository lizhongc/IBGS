...
# ibgs_diag matches base-stats and inline references
set.seed(101)
n <- 2000
...
stopifnot(isTRUE(all.equal(as.numeric(d$acf), ac_ref, tolerance = 1e-8)))
stopifnot(isTRUE(all.equal(d$acf.lag, 0:30)))

# effective size vs n * var / spectrum0.ar
ess_ref <- n * var(x) / spec0_ref(x)
stopifnot(isTRUE(all.equal(d$ess, ess_ref, tolerance = 1e-5)))

# Geweke z and Gelman-Rubin R-hat vs the inline references
stopifnot(isTRUE(all.equal(d$geweke, geweke_ref(x), tolerance = 1e-5)))
gr <- gelman_ref(x, 4)
stopifnot(isTRUE(all.equal(d$gelman[1], unname(gr["point"]), tolerance = 1e-6)))
stopifnot(isTRUE(all.equal(d$gelman[2], unname(gr["upper"]), tolerance = 1e-6)))

# ibgs_diag shrink data grows and is sane
set.seed(202)
x  <- as.numeric(arima.sim(n = 1000, list(ar = 0.4)))
d  <- .Call("ibgs_diag", as.double(x), 4L, 20L, 15L, PACKAGE = "IBGS")
sh <- d$shrink
stopifnot(length(sh$iter) >= 1)
stopifnot(all(diff(sh$iter) > 0))                 # endpoints increase
stopifnot(sh$iter[length(sh$iter)] == 1000)       # last uses the whole chain
stopifnot(all(sh$median >= 1 - 1e-8, na.rm = TRUE))

# an IBGS fit carries tidy convergence diagnostics
set.seed(7)
...
stopifnot(all(c("gelman", "geweke", "ess", "autocorr", "shrink") %in% names(cv)))
stopifnot(identical(names(cv$gelman), c("point", "upper")))
stopifnot(is.na(cv$gelman[["point"]]) || cv$gelman[["point"]] >= 1 - 1e-8)
stopifnot(is.na(cv$ess) || cv$ess > 0)
stopifnot(isTRUE(all.equal(unname(cv$autocorr[["0"]]), 1, tolerance = 1e-8)))
stopifnot(inherits(cv$shrink, "data.frame"))

# summary prints a convergence block and the new plots run
set.seed(8)
dat <- sim_gauss(n = 120, p = 30)
fit <- glmIBGS(dat$y, dat$x, criterion = "BIC", n.draws = 120)

out <- capture.output(print(summary(fit)))
stopifnot(any(grepl("Convergence diagnostics", out)))

pdf(NULL)
stopifnot(inherits(plot(fit, which = "gelman"), "IBGS"))
stopifnot(inherits(plot(fit, which = "autocorr"), "IBGS"))
invisible(dev.off())
